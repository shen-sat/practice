class Controller
	...
	def read_rover_commands(pos_str, instruct_str)
		res = ""
		#loop do
		...
		#instruct = gets.chomp
		res += Rover.new(pos_input[0], pos_input[1], pos_input[2], parser.validate_instruct(instruct_str)).end_position + "\n"
		#end
		if res.split(" ").to_a[0].to_i > plateau.width || res.split(" ").to_a[1].to_i > plateau.height
			raise "Rover will move beyond plateau"
		end		
		res
	end
